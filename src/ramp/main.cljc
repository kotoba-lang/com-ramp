(ns ramp.main
  "Kotodama WASM entrypoint for the Ramp clean-room actor (L4) — Clojure port.
  ...
  facts (`ramp.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Card"          :plural "cards"          :id-prefix "ramp_car"
    :fields [:state :lastFour :cardholderName :cardholderID :isPhysical] :required [:state]
    :coerce {:isPhysical :bool}  :refs {}}
   {:entity "Transaction"   :plural "transactions"   :id-prefix "ramp_tra"
    :fields [:state :amount :currencyCode :cardID :merchantName :userTransactionTime :settlementDate] :required [:state]
    :coerce {:amount :float}     :refs {}}
   {:entity "User"          :plural "users"          :id-prefix "ramp_use"
    :fields [:email :firstName :lastName :role :status :departmentID :isManager] :required [:email]
    :coerce {:isManager :bool}   :refs {}}
   {:entity "Department"    :plural "departments"    :id-prefix "ramp_dep"
    :fields [:name]                                   :required [:name]
    :coerce {}                     :refs {}}
   {:entity "Reimbursement" :plural "reimbursements" :id-prefix "ramp_rei"
    :fields [:state :amount :currency :type :userID :direction] :required [:state]
    :coerce {:amount :float}     :refs {}}
   {:entity "Bill"          :plural "bills"          :id-prefix "ramp_bil"
    :fields [:status :amount :currencyCode :invoiceNumber :statusSummary :dueAt] :required [:status]
    :coerce {:amount :int}       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ramp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ramp-compat" :tier tier :entities entities} 200])
